...
func (h *Handler) MessageCountsBind() {
	...
	for i, messageCount := range messageCountuuid {
		...
		nowcollectingdata = append(nowcollectingdata, UserDetailWithMessageCount{Id: userdetail.Id, DisplayName: userdetail.DisplayName, Name: userdetail.Name, Homechannnel: userdetail.GetHomeChannel(), TotalMessageCount: int64(messageCount.MessageCount)})
